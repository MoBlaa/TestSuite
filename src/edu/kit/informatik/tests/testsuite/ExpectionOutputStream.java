...
/**
 * OutputStream multiple outputs and only writing if output matches
 * the expected output.
 *
 * @author Moritz Hepp
 * @version 1.0
...
public class ExpectionOutputStream extends PipedOutputStream {
     ...
     */
    public ExpectionOutputStream(List<?> expected, OutputStream str, File logFile) throws IOException {
        super();
     ...
     *
     * @return true, if count of already printed outputs is smaller then
     * the size of the expected lines, false otherwise.
     */
        ...
        if (out.startsWith(TestSuite.ERR_PREF) || out.startsWith(TestSuite.DEF_PREF) || out.matches("\n")) {
            if (out.matches("\n")) {
                if (newLineAllowed || !this.isExpecting()) {
                    ...
                    System.err.println(TestSuite.ERR_PREF + ": expected: "
                            + this.expectations.get(count).toString() + " but got new line!");
                }
            } else {
                this.log.print(out);
                this.log.flush();
            ...
            if (count < expectations.size()) {
                ...
                if (sRep.equals(out) || (sRep.equals("00err") && out.startsWith("Error"))) {
                    this.log.print(out);
                    this.log.flush();
